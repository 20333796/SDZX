...
ARG GHCR_IMAGE_PREFIX=ghcr.io/
FROM ${GHCR_IMAGE_PREFIX}astral-sh/uv:0.12.6 AS uv-binary
FROM ${IMAGE_PREFIX}node:24-slim AS node-binary
FROM ${IMAGE_PREFIX}python:3.13-slim AS runtime
ARG INSTALL_DOCUMENT_CONVERTERS=false
ARG INSTALL_MEDIA_TOOLS=false
COPY --from=uv-binary /uv /uvx /bin/
COPY --from=node-binary /usr/local/bin /usr/local/bin
COPY --from=node-binary /usr/local/lib/node_modules /usr/local/lib/node_modules
COPY --from=node-binary /usr/local/include /usr/local/include
COPY --from=node-binary /usr/local/share /usr/local/share
