...
FROM ${IMAGE_PREFIX}node:24-alpine AS development
WORKDIR /app
...
# 生产阶段
FROM ${IMAGE_PREFIX}node:24-alpine AS build-stage
WORKDIR /app

ARG VITE_BASE_PATH=/geochat/
ARG VITE_API_BASE_PATH=/geochat
...
# 生产环境运行阶段
FROM ${IMAGE_PREFIX}nginx:alpine AS production
