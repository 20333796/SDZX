...
def test_system_chat_model_defaults_to_campus_ollama(key):
    """未配置的对话与快速模型使用校内 Ollama。"""
    assert options.system_options.resolve({})[key] == "local-ollama:qwen3.8:latest"


@pytest.mark.parametrize("key", ["default_model", "fast_model"])
def test_system_chat_model_preserves_saved_selection(key):
    """默认值更新不覆盖管理员已保存的模型选择。"""
    saved_model = "local-ollama:qwen3.8:latest"
