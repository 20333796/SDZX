...
async def test_login_identifier_accepts_uid_phone_and_username(user_session) -> None:
    session, user, _keys, _previous_revocation = user_session
    repository = UserRepository(session)

    assert await repository.get_by_login_identifier(user.uid) is user
    assert await repository.get_by_login_identifier(user.phone_number) is user
    assert await repository.get_by_login_identifier(user.username) is user
    assert await repository.get_by_login_identifier("missing-user") is None
