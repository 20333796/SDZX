...
async def login_for_access_token(
    ...
    # 支持用户名、UID 和手机号登录；仓储层按唯一标识优先级消除歧义。
...
@auth.post("/register", response_model=Token, status_code=status.HTTP_201_CREATED)
async def register_public_user(
    registration: PublicRegistration,
    request: Request,
    db: AsyncSession = Depends(get_db),
):
    """注册普通学习账户，并建立与统一门户共享的登录会话。"""
    is_valid, error_msg = validate_username(registration.username)
    if not is_valid:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=error_msg)

    if registration.phone_number and not is_valid_phone_number(registration.phone_number):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="手机号格式不正确")

    user_repository = UserRepository(db)
    if await user_repository.get_by_username(registration.username):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="该用户名已注册",
            headers={"X-Registration-Conflict": "username"},
        )

    if registration.phone_number and await user_repository.exists_by_phone(registration.phone_number):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="该手机号已注册",
    ...
    uid = generate_unique_uid(registration.username, existing_uids)
    ...
    return {
        "access_token": AuthUtils.create_access_token({"sub": str(new_user.id)}),
        "token_type": "bearer",
        "user_id": new_user.id,
        "username": new_user.username,
        "uid": new_user.uid,
        "phone_number": new_user.phone_number,
        "avatar": normalize_public_minio_url(new_user.avatar),
        "role": new_user.role,
