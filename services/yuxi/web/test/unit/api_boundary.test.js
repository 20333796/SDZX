...
test('公开登录 503 明确标记认证服务不可用', async () => {
  await withServer(async (server) => {
    globalThis.fetch = async () =>
    ...
    await assert.rejects(authApi.login({ loginId: 'someone', password: 'secret' }), (error) => {
      assert.equal(error.message, '认证服务暂不可用，请稍后重试')
...
test('公开登录网络失败明确标记无法连接认证服务', async () => {
  await withServer(async (server) => {
    globalThis.fetch = async () => {
    ...
    await assert.rejects(authApi.login({ loginId: 'someone', password: 'secret' }), (error) => {
      assert.equal(error.message, '无法连接认证服务，请检查服务状态或网络连接')
...
test('公开注册 409 区分手机号已注册且不记录系统异常', async () => {
  await withServer(async (server) => {
    const logged = []
    ...
    try {
      ...
      globalThis.fetch = async () =>
        new Response(JSON.stringify({ detail: '该手机号已注册' }), {
          status: 409,
          headers: {
            'content-type': 'application/json',
            'X-Registration-Conflict': 'phone'
          }
        })
      ...
      await assert.rejects(
        ...
        (error) => {
          assert.equal(error.message, '该手机号已注册，请直接登录')
          assert.equal(error.status, 409)
          assert.equal(error.kind, 'already_registered')
          return true
        }
      )
