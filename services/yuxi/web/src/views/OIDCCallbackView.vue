...
const consumePortalReturnUrl = () => {
  const value = sessionStorage.getItem('portal_return')
  sessionStorage.removeItem('portal_return')
  if (!value) return ''
...
const handleCallback = async () => {
  try {
    ...
    const portalReturnUrl = consumePortalReturnUrl()
    ...
    setTimeout(async () => {
      if (portalReturnUrl) {
        window.location.replace(portalReturnUrl)
        return
      }
      // 跳转
      if (redirectPath === '/') {
