...
const configuredApiBasePath = String(import.meta.env.VITE_API_BASE_PATH || '').trim()
const apiBasePath = configuredApiBasePath.replace(/\/+$/, '')

export function withApiBasePath(url) {
  if (!apiBasePath || !url.startsWith('/api/')) return url
  return `${apiBasePath}${url}`
}
...
function publicErrorMessage(url, status, headers, requiresAuth) {
  ...
  if (status === 401) {
    ...
    return path.endsWith('/api/auth/token') ? '用户名或密码错误' : '认证请求失败'
  }
  if (status === 403) return '没有权限执行此操作'
  if (status === 404) return '请求资源不存在'
  if (status === 409 && path.endsWith('/api/auth/register')) {
    const conflict = headers.get('x-registration-conflict')
    if (conflict === 'phone') return '该手机号已注册，请直接登录'
    if (conflict === 'username') return '该用户名已注册，请直接登录'
    return '该账户已注册，请直接登录'
  }
  ...
  if (status >= 500 && path.endsWith('/api/auth/token')) return '认证服务暂不可用，请稍后重试'
...
function classifyApiError(url, status) {
  const path = safeRequestMetadata(url, {}).path
  const isLoginRequest = path.endsWith('/api/auth/token')
  const isRegistrationRequest = path.endsWith('/api/auth/register')

  if (isLoginRequest && status === 401) return 'credentials'
  if (isLoginRequest && status >= 500) return 'service_unavailable'
  if (isRegistrationRequest && status === 409) return 'already_registered'
...
export async function apiRequest(url, options = {}, requiresAuth = true, responseType = 'json') {
  try {
    const requestUrl = withApiBasePath(url)
    ...
    if (!response.ok) {
      // 尝试解析错误信息
      const errorMessage = publicErrorMessage(requestUrl, response.status, response.headers, requiresAuth)
      let errorData = null

      const errorKind = classifyApiError(requestUrl, response.status)
      if (!['credentials', 'account_locked', 'already_registered'].includes(errorKind)) {
        console.error('API请求失败:', safeRequestMetadata(requestUrl, requestOptions, response))
      }
      ...
      try {
        ...
        if (response.status === 422) {
          console.error('API请求校验失败:', safeRequestMetadata(requestUrl, requestOptions, response))
        }
      } catch {
        // 如果无法解析JSON，使用默认错误信息
        console.error('API错误响应无法解析:', safeRequestMetadata(requestUrl, requestOptions, response))
      }
    ...
    const requestUrl = withApiBasePath(url)
    const requestPath = safeRequestMetadata(requestUrl, options).path
    const isLoginRequest = requestPath.endsWith('/api/auth/token')
    const isNetworkError = error instanceof TypeError && error?.status == null

    if (isNetworkError) {
      const networkError = new Error(
        isLoginRequest ? '无法连接认证服务，请检查服务状态或网络连接' : '无法连接服务，请检查网络连接'
      )
      networkError.kind = 'network'
      networkError.status = null
      console.error('API服务连接失败:', safeRequestMetadata(requestUrl, options))
      throw networkError
    }

    if (error.name !== 'AbortError') {
      if (!['credentials', 'account_locked', 'already_registered'].includes(error?.kind)) {
        console.error('API请求异常:', {
          ...safeRequestMetadata(requestUrl, options),
